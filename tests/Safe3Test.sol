...
contract Safe3 is ISafe3 {
    ...
    function redeemLock(bytes memory _pubkey, bytes memory _sig) public {
        ...
        for(uint i = 0; i < locks[keyID].length; i++) {
            ...
            uint lockID = test.getAccountManager().fromSafe3(safe4Addr, info.amount, lockDay, remainLockHeight);
            if(info.isMN) {
                test.getMasterNode().fromSafe3(safe4Addr, info.amount, lockDay, lockID);
            }
        }
    }
