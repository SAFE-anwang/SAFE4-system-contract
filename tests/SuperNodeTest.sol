...
contract SuperNodeTest is ISuperNode {
    using SafeMath for uint256;

    uint internal constant TOTAL_CREATE_AMOUNT  = 2000000000000000000;
    uint internal constant UNION_CREATE_AMOUNT  = 400000000000000000; // 20%
    uint internal constant APPEND_AMOUNT        = 200000000000000000; // 10%
    ...
    constructor(SystemTest _test) {
        test = _test;
        test.setSuperNode(address(this));

        sn_no = 2;

        // sn1: 0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB
        address addr = AddressUtil.parseAddr("0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB");
        SuperNodeInfo storage info = supernodes[addr];
        info.id = 1;
        info.name = "supernode1";
        info.addr = addr;
        info.creator = addr;
        info.amount = TOTAL_CREATE_AMOUNT;
        info.enode = "enode://11e52467b3bdfbe62b1c47a16be714ca7e5ae117f62453142637019ed2b8daa707860a004d381ad3f73f815f61bdbc688f2f40143239e324a8666d075c95f3f0@192.168.0.201:30303";
        info.ip = "192.168.0.201";
        info.description = "official supernode1";
        info.isOfficial = true;
        info.stateInfo = StateInfo(STATE_START, block.number + 1);
        info.founders.push(MemberInfo(1, addr, 400000000000000000, 0));
        info.founders.push(MemberInfo(2, AddressUtil.parseAddr("0x5B38Da6a701c568545dCfcB03FcB875f56beddC4"), 800000000000000000, 0));
        info.incentivePlan = IncentivePlan(10, 40, 50);
        info.lastRewardHeight = block.number + 1;
        info.createHeight = 0;
        info.updateHeight = 0;

        snIDs.push(info.id);
        snID2addr[info.id] = addr;
        snIP2addr[info.ip] = addr;

        // sn2: 0x617F2E2fD72FD9D5503197092aC168c91465E7f2
        addr = AddressUtil.parseAddr("0x617F2E2fD72FD9D5503197092aC168c91465E7f2");
        info = supernodes[addr];
        info.id = 2;
        info.name = "supernode2";
        info.addr = addr;
        info.creator = addr;
        info.amount = TOTAL_CREATE_AMOUNT;
        info.enode = "enode://22e52467b3bdfbe62b1c47a16be714ca7e5ae117f62453142637019ed2b8daa707860a004d381ad3f73f815f61bdbc688f2f40143239e324a8666d075c95f3f0@192.168.0.202:30303";
        info.ip = "192.168.0.102";
        info.description = "official supernode2";
        info.isOfficial = true;
        info.stateInfo = StateInfo(STATE_START, block.number + 1);
        info.founders.push(MemberInfo(1, addr, 500000000000000000, 0));
        info.founders.push(MemberInfo(2, AddressUtil.parseAddr("0x5B38Da6a701c568545dCfcB03FcB875f56beddC4"), 300000000000000000, 0));
        info.founders.push(MemberInfo(3, addr, 4000000000000000000, 0));
        info.incentivePlan = IncentivePlan(10, 40, 50);
        info.lastRewardHeight = block.number + 1;
        info.createHeight = 0;
        info.updateHeight = 0;
    ...
    function register(bool _isUnion, address _addr, uint _lockDay, string memory _name, string memory _enode, string memory _description, uint _creatorIncentive, uint _partnerIncentive, uint _voterIncentive) public payable {
        ...
        test.getAccountManager().setRecordFreeze(lockID, msg.sender, _addr, _lockDay); // partner's lock id can't register other supernode again
    ...
    function appendRegister(address _addr, uint _lockDay) public payable {
        ...
        test.getAccountManager().setRecordFreeze(lockID, msg.sender, _addr, 90); // partner's lock id can register other supernode after 90 days
    ...
    function turnRegister(address _addr, uint _lockID) public {
        ...
        test.getAccountManager().setRecordFreeze(_lockID, msg.sender, _addr, 90); // partner's lock id can register other masternode after 90 days
    ...
    function changeAddress(address _addr, address _newAddr) public {
        require(exist(_addr), "non-existent supernode");
        require(!test.existNodeAddress(_newAddr), "target address has existed");
