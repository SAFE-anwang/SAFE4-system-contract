...
contract MasterNodeStateTest is INodeState {
    ...
    constructor(SystemTest _test) {
        test = _test;
        test.setMasterNodeState(address(this));
    }
    ...
    function uploadState(uint[] memory _ids, uint8[] memory _states) public {
        ...
        for(uint i = 0; i < _ids.length; i++) {
            uint id = _ids[i];
            if(!test.getMasterNode().existID(id)) {
                continue;
            }
        ...
        for(uint i = 0; i < entries.length; i++) {
            if(_state == entries[i].state) {
                if(++num > test.getSNNum() * 2 / 3) {
                    ...
                    test.getMasterNode().changeState(_id, _state);
                    break;
                }
            }
        }
