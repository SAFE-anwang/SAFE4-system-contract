...
contract AccountManagerTest is IAccountManager {
    ...
    constructor(SystemTest _test) payable {
        test = _test;
        test.setAccountManager(address(this));

        uint id = 0;
        address addr;

        uint startHeight = block.number + 1;
        uint unlockHeight = startHeight + 20;
        uint unfreezeHeight = unlockHeight;

        balances[0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2] = 1 ether;
        balances[0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db] = 1 ether;
        balances[0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB] = 2 ether;
        balances[0x617F2E2fD72FD9D5503197092aC168c91465E7f2] = 2 ether;

        // mn1: 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2
        id++;
        addr = AddressUtil.parseAddr("0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2");
        addr2records[addr].push(AccountRecord(id, addr, 1 ether, 0, startHeight, unlockHeight));
        id2index[id] = 0;
        id2addr[id] = addr;
        id2useinfo[id] = RecordUseInfo(addr, startHeight, unfreezeHeight, address(0), 0, 0);
        ...
        id++;
        addr = AddressUtil.parseAddr("0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB");
        addr2records[addr].push(AccountRecord(id, addr, 2 ether, 0, startHeight, unlockHeight));
        id2index[id] = 0;
        id2addr[id] = addr;
        id2useinfo[id] = RecordUseInfo(addr, startHeight, unfreezeHeight, address(0), 0, 0);
        ...
        if(amount != 0) {
            ...
            for(uint i = 0; i < _ids.length; i++) {
                if(_ids[i] != 0) {
                    ...
                    RecordUseInfo memory useinfo = id2useinfo[_ids[i]];
                    if(record.addr == msg.sender && block.number >= record.unlockHeight && block.number >= useinfo.unfreezeHeight) {
                        test.getSNVote().removeVoteOrApproval(msg.sender, _ids[i]);
                        delRecord(_ids[i]);
                    }
                } else {
        ...
        for(uint i = 0; i < temp_records.length; i++) {
            if(usedAmount + temp_records[i].amount <= _amount) {
                if(temp_records[i].id != 0) {
                    test.getSNVote().removeVoteOrApproval(msg.sender, temp_records[i].id);
    ...
    function fromSafe3(address _addr, uint _amount, uint _lockDay, uint _remainLockHeight) public /*onlySafe3Contract*/ returns (uint) {
        ...
        uint unlockHeight = startHeight + _remainLockHeight;
        uint id = ++record_no;
        AccountRecord[] storage records = addr2records[_addr];
    ...
    function setRecordFreeze(uint _id, address _addr, address _target, uint _day) public /*onlyMnOrSnContract*/ {
        ...
        if(_day == 0) {
            ...
            useinfo.unfreezeHeight = useinfo.freezeHeight + _day.mul(5);
    ...
    function setRecordVote(uint _id, address _addr, address _target, uint _day) public /*onlySNVoteContract*/ {
        ...
        if(_day == 0) {
            ...
            useinfo.releaseHeight = useinfo.voteHeight + _day.mul(5);
    ...
    function addLockDay(uint _id, uint _day) public {
        ...
        record.unlockHeight = record.startHeight + record.lockDay.mul(5);
    ...
    function addRecord(address _addr, uint _amount, uint _lockDay) internal returns (uint) {
        ...
        uint unlockHeight = startHeight + _lockDay.mul(5);
        uint id = ++record_no;
        AccountRecord[] storage records = addr2records[_addr];
        records.push(AccountRecord(id, _addr, _amount, _lockDay, startHeight, unlockHeight));
        id2index[id] = records.length - 1;
