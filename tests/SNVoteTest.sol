...
contract SNVote is ISNVote {
    ...
    constructor(SystemTest _test) {
        test = _test;
        test.setSNVote(address(this));
    }

    function voteOrApproval(bool _isVote, address _dstAddr, uint[] memory _recordIDs) public {
        require(!test.isSN(msg.sender), "caller can't be supernode");
        if(_isVote) {
            require(test.isSN(_dstAddr), "invalid supernode address");
        } else {
            require(test.isMN(_dstAddr), "invalid proxy address");
        }
        uint id = 0;
        for(uint i = 0; i < _recordIDs.length; i++) {
            ...
            if(_recordIDs[i] == 0) {
                ...
                id = test.getAccountManager().moveID0(msg.sender);
            }
    ...
    function proxyVote(address _snAddr) public {
        require(test.isMN(msg.sender), "caller isn't proxy");
    ...
    function getProxiedRecordIDs4Voter(address _voterAddr) public view returns (uint[] memory retIDs) {
        ...
        for(uint i = 0; i < ids.length; i++) {
            id = ids[i];
            if(test.isMN(id2record[id].dstAddr)) {
                count++;
            }
        }
        if(count == 0) {
    ...
    function getVoters4Proxy(address _proxyAddr) public view returns (address[] memory retAddrs, uint[] memory retNums) {
        require(test.isMN(_proxyAddr), "invalid proxy");
    ...
    function add4Dst(address _dstAddr, address _voterAddr, uint _recordID, uint _amount, uint _num) internal {
        ...
        if(!flag) {
            ...
            if(test.isSN(_dstAddr)) {
                test.getSuperNode().changeVoter(_dstAddr, _voterAddr, _recordID, _amount, 1);
            }
        }
    ...
    function add(address _voterAddr, address _dstAddr, uint _recordID) internal {
        IAccountManager.AccountRecord memory record = test.getAccountManager().getRecordByID(_recordID);
        require(record.addr == _voterAddr, "caller isn't owner of record");

        IAccountManager.RecordUseInfo memory useinfo = test.getAccountManager().getRecordUseInfo(_recordID);
        require(block.number >= useinfo.releaseHeight, "record is voted, need wait for release");

        uint amount = record.amount;
        uint num = amount;
        if(test.isMN(useinfo.specialAddr)) {
        ...
        // freeze record
        if(test.isSN(_dstAddr)) { // vote
            test.getAccountManager().setRecordVote(_recordID, _voterAddr, _dstAddr, 1);
            test.getSuperNode().changeVoteInfo(_dstAddr, amount, num, 1);
    ...
    function remove4Dst(address _dstAddr, address _voterAddr, uint _recordID, uint _amount, uint _num) internal {
        ...
        if(recordIDs.length == 0) {
            ...
            if(test.isSN(_dstAddr)) {
                test.getSuperNode().changeVoter(_dstAddr, _voterAddr, _recordID, _amount, 0);
            }
        } else {
            // remove amount & votenum
            detail.totalAmount -= _amount;
    ...
    function remove(address _voterAddr, uint _recordID) internal {
        ...
        // unfreeze record
        if(test.isSN(dstAddr)) { // vote
            test.getAccountManager().setRecordVote(_recordID, _voterAddr, dstAddr, 0);
            test.getSuperNode().changeVoteInfo(dstAddr, amount, num, 0);
