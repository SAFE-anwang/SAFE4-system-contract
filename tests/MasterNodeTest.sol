...
contract MasterNodeTest is IMasterNode {
    using SafeMath for uint256;
    uint internal constant TOTAL_CREATE_AMOUNT  = 1000000000000000000;
    uint internal constant UNION_CREATE_AMOUNT  = 200000000000000000; // 20%
    uint internal constant APPEND_AMOUNT        = 100000000000000000; // 10%
    ...
    constructor(SystemTest _test) {
        test = _test;
        test.setMasterNode(address(this));

        mn_no = 2;

        // mn1: 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2
        address addr = AddressUtil.parseAddr("0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2");
        MasterNodeInfo storage info = masternodes[addr];
        info.id = 1;
        info.addr = addr;
        info.creator = addr;
        info.amount = TOTAL_CREATE_AMOUNT;
        info.enode = "enode://111956d5d5f89c5d9d7a6d4402afa5eac41568ffe32d4e05bd63ba440329c866ecb8f60e275336a3b35a58eca352844445344eb4a322733496c811732fb0d903@192.168.0.101:30303";
        info.ip = "192.168.0.101";
        info.description = "official masternode1";
        info.isOfficial = true;
        info.stateInfo = StateInfo(1, block.number + 1);
        info.founders.push(MemberInfo(1, addr, 400000000000000000, 0));
        info.founders.push(MemberInfo(2, AddressUtil.parseAddr("0x5B38Da6a701c568545dCfcB03FcB875f56beddC4"), 1000000000000000000, 0));
        info.incentivePlan = IncentivePlan(50, 50, 0);
        info.lastRewardHeight = block.number + 1;
        info.createHeight = 0;
        info.updateHeight = 0;

        mnIDs.push(info.id);
        mnID2addr[info.id] = addr;
        mnIP2addr[info.ip] = addr;

        
        // mn2: 0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db
        addr = AddressUtil.parseAddr("0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db");
        info = masternodes[addr];
        info.id = 2;
        info.addr = addr;
        info.creator = addr;
        info.amount = TOTAL_CREATE_AMOUNT;
        info.enode = "enode://222956d5d5f89c5d9d7a6d4402afa5eac41568ffe32d4e05bd63ba440329c866ecb8f60e275336a3b35a58eca352844445344eb4a322733496c811732fb0d903@192.168.0.102:30303";
        info.ip = "192.168.0.202";
        info.description = "official masternode2";
        info.isOfficial = true;
        info.stateInfo = StateInfo(1, block.number + 1);
        info.founders.push(MemberInfo(1, addr, 300000000000000000, 0));
        info.founders.push(MemberInfo(2, AddressUtil.parseAddr("0x5B38Da6a701c568545dCfcB03FcB875f56beddC4"), 600000000000000000, 0));
        info.founders.push(MemberInfo(3, addr, 200000000000000000, 0));
        info.incentivePlan = IncentivePlan(50, 50, 0);
        info.lastRewardHeight = block.number + 1;
        info.createHeight = 0;
        info.updateHeight = 0;
    ...
    function register(bool _isUnion, address _addr, uint _lockDay, string memory _enode, string memory _description, uint _creatorIncentive, uint _partnerIncentive) public payable {
        ...
        require(!test.existNodeAddress(_addr), "existent address");
        require(!test.existNodeIP(ip), "existent ip");
        uint lockID = test.getAccountManager().deposit{value: msg.value}(msg.sender, _lockDay);
        create(_addr, lockID, msg.value, _enode, ip, _description, IncentivePlan(_creatorIncentive, _partnerIncentive, 0));
        test.getAccountManager().setRecordFreeze(lockID, msg.sender, _addr, _lockDay); // creator's lock id can't register other masternode again
    ...
    function appendRegister(address _addr, uint _lockDay) public payable {
        ...
        test.getAccountManager().setRecordFreeze(lockID, msg.sender, _addr, 30); // partner's lock id can register other masternode after 30 days
        emit MNAppendRegister(_addr, msg.sender, msg.value, _lockDay, lockID);
    }

    function turnRegister(address _addr, uint _lockID) public {
        require(exist(_addr), "non-existent masternode");
        IAccountManager.AccountRecord memory record = test.getAccountManager().getRecordByID(_lockID);
        ...
        IAccountManager.RecordUseInfo memory useinfo = test.getAccountManager().getRecordUseInfo(_lockID);
        require(block.number >= useinfo.unfreezeHeight, "record is freezen");
        append(_addr, _lockID, record.amount);
        test.getAccountManager().setRecordFreeze(_lockID, msg.sender, _addr, 30); // partner's lock id can register other masternode after 30 days
    ...
    function reward(address _addr) public payable /*onlySystemRewardContract*/ {
        ...
        for(uint i = 0; i < count; i++) {
            test.getAccountManager().reward{value: tempAmounts[i]}(tempAddrs[i]);
    ...
    function fromSafe3(address _addr, uint _amount, uint _lockDay, uint _lockID) public /*onlySafe3Contract*/ {
        ...
        test.getAccountManager().setRecordFreeze(_lockID, _addr, _addr, _lockDay); // creator's lock id can't register other masternode again
        emit MNRegister(_addr, msg.sender, _amount, _lockDay, _lockID);
    }

    function changeAddress(address _addr, address _newAddr) public {
        require(exist(_addr), "non-existent masternode");
        require(!test.existNodeAddress(_newAddr), "existent new address");
    ...
    function changeEnode(address _addr, string memory _newEnode) public {
        ...
        require(!test.existNodeIP(ip), "existent ip of new enode");
        require(msg.sender == masternodes[_addr].creator, "caller isn't masternode creator");
